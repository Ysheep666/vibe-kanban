...
export type CursorMcpRole = "assistant" | "user" | "system";

export type CursorMcpMessage = { id: string, 
/**
 * vk_session_id when adopted, else `None` (lobby).
 */
vk_session_id: string | null, 
/**
 * Always set — friendly bridge id.
 */
bridge_session_id: string, role: CursorMcpRole, body: string, created_at: string, };

export type CursorMcpWaitInfo = { request_id: string, 
/**
 * vk_session_id when adopted, else `None` (lobby).
 */
vk_session_id: string | null, bridge_session_id: string, message: string, prompt: string | null, enqueued_at: string, renew_deadline_at: string, };

export type CursorMcpSessionSnapshot = { session_id: string, bridge_session_id: string | null, messages: Array<CursorMcpMessage>, pending_waits: Array<CursorMcpWaitInfo>, bridge_connected: boolean, };

export type CursorMcpPatch = { "type": "snapshot", "payload": CursorMcpSessionSnapshot } | { "type": "message_appended", "payload": CursorMcpMessage } | { "type": "wait_enqueued", "payload": CursorMcpWaitInfo } | { "type": "wait_resolved", "payload": { request_id: string, } } | { "type": "bridge_connected", "payload": boolean };

export type BridgeInbound = { "type": "register", version: string, label: string | null, } | { "type": "wait", request_id: string, bridge_session_id: string, message: string, prompt: string | null, title: string | null, } | { "type": "cancel_wait", request_id: string, } | { "type": "ping" };

export type BridgeOutbound = { "type": "registered" } | { "type": "wait_result", request_id: string, text: string, session_id: string, } | { "type": "error", request_id: string | null, message: string, } | { "type": "pong" };

export type InboxBridgeInfo = { bridge_id: string, label: string | null, };

export type InboxLobbyItem = { bridge_session_id: string, bridge_label: string | null, title: string | null, first_message: string | null, last_activity_at: string, pending_count: number, };

export type InboxSnapshot = { bridges: Array<InboxBridgeInfo>, lobby: Array<InboxLobbyItem>, };

export type InboxPatch = { "type": "snapshot", "payload": InboxSnapshot } | { "type": "bridges_changed", "payload": { count: number, } } | { "type": "session_updated", "payload": InboxLobbyItem } | { "type": "session_adopted", "payload": { bridge_session_id: string, vk_session_id: string, } } | { "type": "session_removed", "payload": { bridge_session_id: string, } };
 ...
 */
mcp_binary_path: string | null, executors: { [key in BaseCodingAgent]?: ExecutorProfile }, };
...
export type ResolveBody = { text: string, };

export type LaunchConfig = { mcp_json_snippet: string, server_name: string, binary_path: string | null, binary_exists: boolean, binary_source: string, };

export type AdoptLobbyBody = { 
/**
...
export type AdoptLobbyResponse = { bridge_session_id: string, vk_session_id: string, };
...
export enum BaseCodingAgent { CLAUDE_CODE = "CLAUDE_CODE", AMP = "AMP", GEMINI = "GEMINI", CODEX = "CODEX", OPENCODE = "OPENCODE", CURSOR_AGENT = "CURSOR_AGENT", CURSOR_MCP = "CURSOR_MCP", QWEN_CODE = "QWEN_CODE", COPILOT = "COPILOT", DROID = "DROID" }

export type CodingAgent = { "CLAUDE_CODE": ClaudeCode } | { "AMP": Amp } | { "GEMINI": Gemini } | { "CODEX": Codex } | { "OPENCODE": Opencode } | { "CURSOR_AGENT": CursorAgent } | { "CURSOR_MCP": CursorMcp } | { "QWEN_CODE": QwenCode } | { "COPILOT": Copilot } | { "DROID": Droid };
...
export type ExecutorProfile = { recently_used_models?: ExecutorRecentModels | null, } & ({ [key in string]?: { "CLAUDE_CODE": ClaudeCode } | { "AMP": Amp } | { "GEMINI": Gemini } | { "CODEX": Codex } | { "OPENCODE": Opencode } | { "CURSOR_AGENT": CursorAgent } | { "CURSOR_MCP": CursorMcp } | { "QWEN_CODE": QwenCode } | { "COPILOT": Copilot } | { "DROID": Droid } });
...
export type CursorMcp = { append_prompt: AppendPrompt, base_command_override?: string | null, additional_params?: Array<string> | null, env?: { [key in string]?: string } | null, };
